...
const _issueVerificationToken = async (user, { baseUrl } = {}) => {
  ...
  // Fire-and-forget so a slow/broken SMTP never blocks registration —
  // the token is already persisted above.
  Promise.resolve()
    .then(() =>
      sendVerificationEmail({ to: user.email, name: user.name, verifyUrl, expiresInHours })
    )
    .catch((err) => logger.error('Failed to send verification email', { message: err.message }));
...
const register = async ({ name, email, phone, password, currency, timezone }, ctx = {}) => {
  ...
  const user = await User.create({
    ...
    email,
    phone,
    ...(phone ? { phoneNormalized: normalize(phone) } : {}),
    password,
    preferences: {
      ...(currency && { currency }),
      ...(timezone && { timezone }),
