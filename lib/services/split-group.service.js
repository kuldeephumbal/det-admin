...
const invite = async (userId, groupId, body) => {
  ...
  // Phone with no DET user yet → create a placeholder user so they're a
  // full, splittable member IMMEDIATELY (even if they never accept). The
  // mobile app opens the inviter's SMS composer with a draft invite — we
  // don't send SMS server-side. When the person later signs up, linking
  // by phone is a follow-up (App Links); for now they exist as a member.
  const digits = normPhone.replace(/[^0-9]/g, '');
  const placeholderEmail = `${digits}@det-invite.local`;
  ...
  return toPublicMember(m, { user: ghost });
};
